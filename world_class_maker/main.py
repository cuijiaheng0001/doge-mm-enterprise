...
class WorldClassMarketMaker:
    ...
    async def _generate_quotes(self, ticker: TickerData):
        ...
        try:
            ...
            # 多层级报价配置 - 5个价格档位（ChatGPT成功版本的配置）
            levels = [
                {'offset': Decimal('0.05'), 'qty_ratio': Decimal('0.3')},  # 最紧档位 - 30%资金
                {'offset': Decimal('0.15'), 'qty_ratio': Decimal('0.25')}, # 次紧档位 - 25%资金   
                {'offset': Decimal('0.30'), 'qty_ratio': Decimal('0.20')}, # 中等档位 - 20%资金
                {'offset': Decimal('0.50'), 'qty_ratio': Decimal('0.15')}, # 远端档位 - 15%资金
                {'offset': Decimal('0.80'), 'qty_ratio': Decimal('0.10')}  # 最远档位 - 10%资金
            ]
            
            current_time = int(time.time() * 1000)  # 毫秒级时间戳确保唯一性
            orders_to_place = []  # 收集要下单的订单
            
            # 生成多层级买单
            for i, level in enumerate(levels):
                buy_price = mid - spread * level['offset']
                buy_qty = min(
                    usdt_available * level['qty_ratio'] / buy_price,  # 基于USDT余额
                    Decimal('150')  # 单笔最大数量限制
                )
                
                if buy_qty >= Decimal('20'):  # 最小交易量阈值
                    buy_feasible, _ = self.reservation_model.check_order_feasible(
                        'BUY', buy_qty, buy_price
                    )
                    
                    if buy_feasible:
                        order_id = f"buy_L{i}_{current_time}_{self.metrics['orders_created']}"
                        if self.reservation_model.reserve_for_order(order_id, 'BUY', buy_qty, buy_price):
                            ...
                            logger.debug(
                                "[WorldClassMaker] ✅ Buy L%d: price=%s qty=%s", 
                                i+1, buy_price, buy_qty
                            )
            
            # 生成多层级卖单
            for i, level in enumerate(levels):
                sell_price = mid + spread * level['offset']
                sell_qty = min(
                    doge_available * level['qty_ratio'],  # 基于DOGE余额
                    Decimal('150')  # 单笔最大数量限制
                )
                
                if sell_qty >= Decimal('20'):  # 最小交易量阈值
                    sell_feasible, _ = self.reservation_model.check_order_feasible(
                        'SELL', sell_qty, sell_price
                    )
                    
                    if sell_feasible:
                        order_id = f"sell_L{i}_{current_time}_{self.metrics['orders_created']}"
                        if self.reservation_model.reserve_for_order(order_id, 'SELL', sell_qty, sell_price):
                            self.metrics['orders_created'] += 1
                            ...
                            logger.debug(
                                "[WorldClassMaker] ✅ Sell L%d: price=%s qty=%s", 
                                i+1, sell_price, sell_qty
                            )
            
            # 批量下单到交易所
            if orders_to_place:
                logger.info("[WorldClassMaker] 📤 准备下单 %d 个订单到交易所", len(orders_to_place))
                
                # 使用连接器批量下单
                for order in orders_to_place:
                    try:
                        # 使用正确的API调用方法
                        result = await self.turbo_connector.create_order_v2(
                            symbol=order['symbol'],
                            side=order['side'],
                            quantity=order['quantity'],
                            price=order['price'],
                            order_type='LIMIT_MAKER',  # 使用LIMIT_MAKER类型
                            client_order_id=order['client_order_id']
                        )
                        
                        if result and 'orderId' in result:
                            logger.info(
                                "[WorldClassMaker] ✅ 订单已下: %s %s %.2f@%.5f orderId=%s",
                                order['side'], order['symbol'], order['quantity'], 
                                order['price'], result['orderId']
                            )
                        else:
                            logger.error(
                                "[WorldClassMaker] ❌ 下单失败: %s %s %.2f@%.5f",
                                order['side'], order['symbol'], order['quantity'], order['price']
                            )
                            # 释放预留资金
                            self.reservation_model.release_reservation(order['client_order_id'])
                            
                    except Exception as e:
                        logger.error(
                            "[WorldClassMaker] ❌ 下单异常: %s %s %.2f@%.5f - %s",
                            order['side'], order['symbol'], order['quantity'], 
                            order['price'], str(e)
                        )
                        # 释放预留资金
